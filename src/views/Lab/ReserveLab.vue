        ...
        <div class="reserve-body">
          ...
          <el-select v-model="experimentChoose" class="input-style" filterable placeholder="选择实验 | 可输入关键词查找"
            ...
            <el-option-group label="实验">
              <el-option v-for="item in allExperiments" :key="item.Eid" :label="item.Name"
        ...
        <el-card shadow="never">
          ...
          <div>
            <el-table
              ...
              <el-table-column label="实验编号" prop="ExperimentID"></el-table-column>
              <el-table-column label="实验名称" prop="ExperimentName"></el-table-column>
              ...
              <el-table-column align="right">
                ...
                <template slot-scope="scope">
                  <el-button
                    disabled
                    size="mini"
                    @click="handleEdit(scope.$index, scope.row)">修改预约
                  </el-button>
                  <el-button
                    size="mini"
                    type="danger"
...
export default {
  ...
  methods: {
    ...
    handleDelete(index, row) {
      this.$confirm('确认取消预约？')
        .then(res => {
          deleteRecord(row.ExperimentID, row.UserID, row.Date, row.Time, row.Site).then(
            res => {
              getUserRecords(this.$store.state.user.studentID).then(res => {
    ...
    changeSelected(value) {
      ...
      getExperimentByEID(value).then(
        res => {
          this.siteSize = res.data.data.SiteSize
        }
      )
    },
    reserveSubmit() {
      if (this.timeChoose === "上午(8:50-11:50)") this.timeChoose = 0
      else if (this.timeChoose === "下午(14:30-17:30)") this.timeChoose = 1
      else if (this.timeChoose === "晚上(18:30-21:30)") this.timeChoose = 2
  ...
  created() {
    getAllExperiments().then(res => {
      this.allExperiments = res.data.all
    })
    getUserRecords(this.$store.state.user.studentID).then(
      res => {
        console.log(res)
        this.reserveData = res.data.data
      }
      ...
      if (val.experimentChoose !== '' && val.dateChoose !== '' && val.timeChoose !== '') {
        ...
        getSiteSelected(this.experimentChoose, this.dateChoose, this.timeChoose).then(
          res => {
            ...
            this.siteSelected = res.data.array
          }
        )
      }
    }
